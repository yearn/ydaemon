...
** indexNewExperimentalVaultsWrapper. It uses the WS connection to listen to the events.
...
**************************************************************************************************/
func indexNewExperimentalVaults(
	chainID uint64,
	...
	**********************************************************************************************/
	...
	channel := make(chan *contracts.Yregistryv2NewExperimentalVault)
	watcher, err := currentVault.WatchNewExperimentalVault(
	...
	**********************************************************************************************/
	for {
		...
		case log := <-channel:
			...
			logs.Info(`New Experimental Vault detected by indexer!`)
...
** Returns nothing
**************************************************************************************************/
func indexNewExperimentalVaultsWrapper(
	chainID uint64,
	registryAddress ethcommon.Address,
	registryVersion uint64,
	...
	**********************************************************************************************/
	...
	err := error(nil)
	for {
		lastSyncedBlock, shouldRetry, err = indexNewExperimentalVaults(
			chainID,
			registryAddress,
			registryVersion,
			lastSyncedBlock,
	...
	**********************************************************************************************/
	if delay > 0 {
		for {
			...
			filterNewExperimentalVault(chainID, common.FromAddress(registryAddress), registryVersion, lastSyncedBlock, &vaultsList, nil)
			...
			if shouldRetry {
				indexNewExperimentalVaultsWrapper(chainID, registryAddress, registryVersion, lastSyncedBlock, 0)
				if delay > 60 {
...
func IndexNewExperimentalVaults(chainID uint64) {
	for _, registry := range env.YEARN_REGISTRIES[chainID] {
		go indexNewExperimentalVaultsWrapper(chainID, registry.Address.ToAddress(), registry.Version, registry.Activation, 1*time.Minute)
	}
