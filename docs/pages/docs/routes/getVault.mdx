# GetVaults
REST API endpoint to get one specific vault for a given chainID. You can check the [getSupportedChains](/docs/routes/getSupportedChains) endpoint to get the list of supported chains.
...
If the address is invalid or the vault does not exists, the `400` error code is returned. Otherwise, the data is merged with:
...
- `address`: The address of the vault you want to get. Must be provided in the URL.
...
- `?strategiesCondition=withDetails|noDetails`: Indicate if we should also query and serve the details about the strategies. If `noDetails` is set, the `Details` field will be ignored. Default is `noDetails`.
