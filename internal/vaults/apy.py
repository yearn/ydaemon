...
def calculate_simple(vault, gauge: Gauge, samples: ApySamples) -> Apy:
    ...
    baseAssetPrice = magic.get_price(gauge.lp_token, block=block)
    if not baseAssetPrice:
        raise ValueError(f"Error! Could not find price for {gauge.lp_token} at block {block}")

    crvPrice = magic.get_price(curve.crv, block=block)
    poolPrice = gauge.pool.get_virtual_price(block_identifier=block)
    ...
    base_apr = (
        ...
        * (PER_MAX_BOOST / poolPrice)
        * crvPrice
    ) / baseAssetPrice

    if y_gauge_balance > 0:
        yBoost = y_working_balance / (PER_MAX_BOOST * y_gauge_balance)
    else:
        yBoost = BOOST[chain.id]
    ...
    if hasattr(gauge.gauge, "reward_contract"):
        ...
        if reward_address != ZERO_ADDRESS:
            reward_apr = rewards(reward_address, poolPrice, baseAssetPrice, block=block)
        ...
        if gauge_reward_token in [ZERO_ADDRESS]:
            ...
            if period_finish < current_time:
                ...
                reward_apr = (SECONDS_PER_YEAR * (rate / 1e18) * token_price) / ((poolPrice / 1e18) * (total_supply / 1e18) * baseAssetPrice)
    else:
        reward_apr = 0
    ...
    if _ConvexVault.is_convex_vault(vault):
        ...
        return cvx_vault.apy(baseAssetPrice, poolPrice, base_apr, pool_apy, management, performance)
    ...
    if isinstance(vault, VaultV2) and len(vault.strategies) == 2: # this vault has curve and convex
        ...
        cvx_apy_data = cvx_vault.get_detailed_apy_data(baseAssetPrice, poolPrice, base_apr)
    else:
        cvx_apy_data = ConvexDetailedApyData()
        crv_debt_ratio = 1

    crv_apr = base_apr * yBoost + reward_apr
    crv_apr_minus_keep_crv = base_apr * yBoost * (1 - crv_keep_crv)
...
class _ConvexVault:
    ...
    def apy(self, baseAssetPrice, poolPrice, base_apr, pool_apy: float, management_fee: float, performance_fee: float) -> Apy:
        """The standard APY data."""
        apy_data = self.get_detailed_apy_data(baseAssetPrice, poolPrice, base_apr)
    ...
    def get_detailed_apy_data(self, baseAssetPrice, poolPrice, base_apr) -> ConvexDetailedApyData:
        ...
        convex_reward_apr = self._get_reward_apr(self._cvx_strategy, cvx_booster, baseAssetPrice, poolPrice, self.block)
    ...
    def _get_cvx_emissions_converted_to_crv(self) -> float:
        ...
        crvPrice = magic.get_price(curve.crv, block=self.block)
        ...
        if supply <= max_supply:
            ...
            converted_cvx = cvx_price / crvPrice
    ...
    def _get_reward_apr(self, cvx_strategy, cvx_booster, baseAssetPrice, poolPrice, block=None) -> float:
        ...
        for x in range(rewards_length):
            ...
            if virtual_rewards_pool.periodFinish() > current_time:
                ...
                reward_apr = (virtual_rewards_pool.rewardRate() * SECONDS_PER_YEAR * reward_token_price) / (baseAssetPrice * (poolPrice / 1e18) * virtual_rewards_pool.totalSupply())
