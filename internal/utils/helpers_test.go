...
func TestReadAllFilesInDir(t *testing.T) {
	_, _, err := ReadAllFilesInDir(META_BASE_PATH+`/strategies/2500/`, `.json`)
	assert.Error(t, err)

	_, _, err = ReadAllFilesInDir(META_BASE_PATH+`/protocols/42161/`, `.json`)
	assert.Error(t, err)

	_, _, err = ReadAllFilesInDir(META_BASE_PATH+`/protocols/250/0xDAO.json`, `.json`)
	assert.Error(t, err)

	_, _, err = ReadAllFilesInDir(META_BASE_PATH+`/protocols/250/`, `.json`)
	assert.Nil(t, err)
}
