...
The yDaemon API is a REST API that provides access to the Yearn data sources. It should provide all the data that the Yearn team needs to build the Yearn ecosystem, based on a system of Daemons, little subprocess (_Goroutines_) that are responsible for fetching, handling, and storing data from the various sources to make them available to the rest of the system.
...
On [`cmd/main.go`](https://github.com/yearn/ydaemon/blob/main/cmd/main.go)
