...
If the address is invalid or the vault does not exist, the `400` error code is returned. Otherwise, the data is merged with:
...
All of these are correctly associated and merged based on the arguments passed to the endpoint, and then returned to the client as a JSON object with the `200` success code.
