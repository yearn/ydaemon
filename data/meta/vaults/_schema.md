...
| Field | Type | Description | Automatic update |
| --- | --- | --- | --- |
| `review` | `int` | To have a unified reviewScore for both internal and external strategies, we assume that the strategist writer itself is either a Source of Trust (internal) or not (external). So all internal strategies always includes that 1 additional source of trust in addition. Together with all other Sources of Trust (SoTs) in this list: internal strategist wrote the strategy, peer reviews, expert peer reviews, ySec security reviews and ySec recurring security review. Each item accounts for 1 SoT point, and any combinations of these gives the number of SoTs a strategy has and thus gives the associated review score: 1 -> 5 SoT; 2 -> 4 SoT; 3 -> 3 SoT; 4 -> 2 SoT; 5 -> 1 SoT | ❌ |
| `testing` | `int` | The testing coverage of the strategy being evaluated. 1 -> 95%+; 2 -> 90%+; 3 -> 80%+; 4 -> 70%+; 5 -> below 70% | ❌ |
| `complexity` | `int` | The sLOC count of the strategy being evaluated. 1 -> 0-150 sLOC; 2 -> 150-300 sLOC; 3 -> 300-450 sLOC; 4 -> 450-600 sLOC; 5 -> 750+ sLOC | ❌ |
| `riskExposure` | `int` | This score aims to find out how much and how often a strategy can be subject to losses. 1 -> Strategy has no lossable cases, only gains, up only.; 2 -> Loss of funds or non recoverable funds up to 0-10% (Example, deposit/withdrawal fees or anything protocol specific); 3 -> Loss of funds or non recoverable funds up to 10-15% (Example, Protocol specific IL exposure, very high deposit/withdrawal fees); 4 -> Loss of funds or non recoverable funds up to 15-70% (Example, adding liquidity to single sided curve stable pools); 5 -> Loss of funds or non recoverable funds up to 70-100% (Example, Leveraging cross assets and got liquidated, adding liquidity to volatile pairs single sided) | ❌ |
| `protocolIntegration` | `int` | The protocols that are integrated into the strategy that is being evaluated. 1 -> Strategy interacts with 1 external protocol; 2 -> Strategy interacts with 2 external protocols; 3 -> Strategy interacts with 3 external protocols; 4 -> Strategy interacts with 4 external protocols; 5 -> Strategy interacts with 5 external protocols | ❌ |
| `centralizationRisk` | `int` | The centralization score of the strategy that is being evaluated. 1 -> Strategy operates without dependency on any privileged roles, ensuring full permissionlessness.; 2 -> Strategy has privileged roles but they are not vital for operations and pose minimal risk of rug possibilities; 3 -> Strategy involves privileged roles but less frequently and with less risk of rug possibilities; 4 -> Strategy frequently depends on off-chain management but has safeguards against rug possibilities by admins; 5 -> Strategy heavily relies on off-chain management, potentially exposing user funds to rug possibilities by admins | ❌ |
| `externalProtocolAudit` | `int` | The public audits count of the external protocols. 1 -> Audit conducted by 4 or more trusted firm or security researcher conducted.; 2 -> Audit conducted by 3 trusted firm or security researcher conducted; 3 -> Audit conducted by 2 trusted firm or security researcher conducted; 4 -> Audit conducted by 1 trusted firm or security researcher conducted; 5 -> No audit conducted by a trusted firm or security researcher | ❌ |
| `externalProtocolCentralisation` | `int` | Measurement of the centralization score of the external protocols. 1 -> Contracts owner is a multisig with known trusted people with Timelock OR Contracts are governanceless, immutable OR Contracts owner can't do any harm to our strategy by setting parameters in external protocol contracts.; 2 -> Contracts owner is a multisig with known trusted people; 3 -> Contracts owner is a multisig with known people but multisig threshold is very low; 4 -> Contracts owner is a multisig but the addresses are not known/hidden OR Contracts owner can harm our strategy by setting parameters in external protocol contracts up to some degree; 5 -> Contracts owner is an EOA or a multisig with less than 4 members OR Contracts are not verified OR Contracts owner can harm our strategy completely | ❌ |
...
| `justificationCommentForRiskScore` | `string` | Justification for given risk score to the strategy. Can be empty. | ❌ |
	...
	"0x00cb87656196dd835b9e4d67018ae0477a1de8c1": {
			...
			"metadata": {
				...
				"riskScore": {
                    ...
                    "justificationCommentForRiskScore": ""
				}
			}
		},
	// Some other vaults
