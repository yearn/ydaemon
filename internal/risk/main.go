...
func computeTVL(t models.TStrategy) (*bigNumber.Float, *bigNumber.Float, *bigNumber.Float) {
	...
	_, amount := helpers.FormatAmount(t.LastEstimatedTotalAssets.String(), int(tokenData.Decimals))
	_tokenPrice, ok := storage.GetPrice(t.ChainID, tokenData.Address)
	if !ok {
		return bigNumber.NewFloat(0), amount, bigNumber.NewFloat(0)
	}
	tvl := bigNumber.NewFloat(0).Mul(amount, _tokenPrice.HumanizedPrice)
	return tvl, amount, _tokenPrice.HumanizedPrice
}
...
func populateRisk(chainID uint64) {
	...
	**********************************************************************************************/
	...
	strategies, _ := storage.ListStrategies(chainID)
