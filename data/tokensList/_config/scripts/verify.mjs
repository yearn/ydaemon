...
const SchemasDirectory = "./data/tokensList/_config/schema/";
const DataDirectory = "./data/tokensList/public/";
	...
	for (let name of fs.readdirSync(directory)) {
		...
		if (stat.isFile() && StandardExtensions.includes(ext)) {
			...
			try {
				...
				if (!schema) {
					if (file.includes("tokenList.json") || file === 'data/tokensList/public/yearn.json') {
						schema = "tokenList";
					} else {
						console.error(
