...
REST API endpoint to list the blacklisted vaults. A vault can be blacklisted for various reasons, but mostly because of failed deployment or tests-only setups.
...
A blacklisted vault will be ignored by the daemon, and will not be considered a valid vault for any query.
